...
Train, test and validation datasets were created. Stratified sampling was done to ensure representativeness of success outcome in all samples. The Valdation dataset was set at 30% of the sample and used in loss and accuracy graphs to aid in assessing the deep learning model fit.
...
The machine learning model program was built to support the four different models and to produce similar output for each model. The user selects the model in the second cell. 

### Comparison of Models

the accuracy curve for nn shows validation accuracy always higher than training. Is there an issue with the sample? maybe not representative of validation? Changed random_state variable. That fixed the problem, but validation loss increases after about 8 epochs, so overfitting. Reduced epochs to 10.
